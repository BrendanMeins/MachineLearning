...
class Agent:
    ...
    def __init__(self, action_count):
        ...
        # gamma: 0.99-0.7, alpha: 0.5, epsilon 0.2 sehr gut
        self.discountrate = 0.95  # gamma
        self.alpha = 0.3# alpha
        self.epsilon = 0.1 # fuer epsilon greedy policy (epsilon = Anteil random)
        self.stateinfos = {}
        self.oldstate = -1
        self.oldaction = -1
    ...
    def cluster(self, state, min, max, cluster_step):

        size = [cluster_step] * len(state)

        win_size = (max - min) / size

        clustered_state = (state - min) / win_size

        res = tuple(np.asarray(clustered_state).astype(int))

        return res

    def done_cluster(self, state, reward, min, max, cluster_step):
        print("Log : Done_Cluster()")
        state = self.cluster(state, min, max, cluster_step)
        self.step(state, reward)
        print()
        return

    def step_cluster(self, state, reward, min, max, cluster_step):
        logging.debug("step(): state: %d, reward: %d", state, reward)
        state = self.cluster(state, min, max, cluster_step)
